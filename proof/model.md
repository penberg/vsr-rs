...
- **Containers.** The Lean keeps the Rust `BTreeMap`s as sorted
  ...
  `DoViewChange`, states its tie-break.
...
that must want a view before a replica sends its `DoViewChange`, so $f$ others plus the
replica itself is $Q$.
...
&\mathsf{DoViewChange}(v, i, \ell, L, n, k) && i\text{'s body: last normal view } \ell \text{, log } L \text{, } n = |L| \\
...
The body (`DoViewChange`) is the triple $(\ell, L, k)$ a `DoViewChange` message carries.
...
| $\mathrm{dvcSent}_r$ | `doViewChangeSent` | sent its `DoViewChange` for $v_r$ already |
| $D_r$ | `doViewChangeFrom` | partial map, id $\mapsto$ body, the `DoViewChange` bodies collected as primary of $v_r$ |
...
cluster step moves into its history, and the ghost `chosenDoViewChanges`, which it
...
reply ever produced, and $\mathit{started}$ (ghost) the set of $(v, D)$
for every view $v$ some primary started with the `DoViewChange` bodies $D$ it chose the log
...
$S_r := \emptyset$; $\mathrm{dvcSent}_r := \mathrm{false}$; $D_r := \emptyset$;
...
$\mathrm{maybeSendDVC}(r)$.

**maybeSendDVC** (`maybeSendDoViewChange`).
If $\mathrm{status}_r = \mathsf{viewchange}$, $\lnot\mathrm{catching}_r$,
$\lnot\mathrm{dvcSent}_r$, and $|S_r| \ge f$:
$\mathrm{dvcSent}_r := \mathrm{true}$; $\mathrm{sendDVC}(r)$.

**sendDVC** (`sendDoViewChange`).
Let $d = (\ell_r, L_r, k_r)$.
If $\mathrm{primary}(v_r) = \mathrm{id}_r$ then
$\mathrm{recordDVC}(r, \mathrm{id}_r, d)$, else $r$ sends
$\mathsf{DoViewChange}(v_r, \mathrm{id}_r, \ell_r, L_r, n_r, k_r)$ to
$\mathrm{primary}(v_r)$.

**recordDVC** (`recordDoViewChange`). $\mathrm{recordDVC}(r, i, d)$:
$D_r[i] := d$. If $|D_r| < Q$ stop. Else:

- $\mathit{best}$ = the body in $D_r$ with the greatest
  $(\ell, |L|)$ in lexicographic order (`bestDoViewChange`); among
  equal keys, the one from the highest replica id.
- $k^* = \max \{\, k : (\ell, L, k) \in D_r \,\}$.
- $r$ starts $v_r$ from $D_r$.
...
- $v < v_r$: drop.
- $v > v_r$: $\mathrm{svc}(r, v)$; $S_r := S_r \cup \{i\}$; $\mathrm{maybeSendDVC}(r)$.
- $v = v_r$, $\mathrm{status}_r = \mathsf{viewchange}$:
  $S_r := S_r \cup \{i\}$; $\mathrm{maybeSendDVC}(r)$.
...
$\mathsf{DoViewChange}(v, i, \ell, L, n, k)$, body $d = (\ell, L, k)$.
Guard: $v \ge v_r$ and $\mathrm{primary}(v) = \mathrm{id}_r$.

- $v > v_r$: $\mathrm{svc}(r, v)$; $\mathrm{recordDVC}(r, i, d)$.
- $v = v_r$, $\mathrm{status}_r = \mathsf{normal}$:
  $r$ sends $\mathsf{StartView}(v_r, L_r, n_r, k_r)$ to $i$.
- $v = v_r$, otherwise: $\mathrm{recordDVC}(r, i, d)$.
...
- $\mathsf{viewchange}$: if the timer fires, $\mathrm{svc}(r, v_r + 1)$.
  ...
  if $\mathrm{dvcSent}_r$, $\mathrm{sendDVC}(r)$ again.
