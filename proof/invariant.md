...
**Durability** (`Durability`). Let $\mathit{participants}$ be the replicas with
$\mathrm{status} \ne \mathsf{recovering}$. For every $r \in \mathit{participants}$
and $i < k_r$:

$$
|\mathit{participants}| + 1 - Q \;\le\; |\{\, o \in \mathit{participants} : L_o[i] = L_r[i] \,\}| .
$$
...
this is a majority. A recovering replica holds nothing and takes part
in no quorum, so it counts on neither side.
...
A `DoViewChange` body is a fragment of the view its sender was *last normal
in*, not of the view the message is for. That is the decision that makes a replica catching
...
in its view; a `DoViewChange` is judged by its last normal view:
...
$(v, D) \in \mathit{started}$ with $|D| \ge Q$, distinct senders, and
$\mathit{best}$ = the best body of $D$, such that $\mathit{best}.L \sqsubseteq L$.

**DvcCoversAcks** (`DvcCoversAcks`). If $(v, D) \in \mathit{started}$
and $(q, (\ell, L, \_)) \in D$, or $\mathrm{Sent}(\_, \mathsf{DoViewChange}(v, q, \ell, L, \_, \_))$, then
$\mathrm{Sent}(\_, \mathsf{PrepareOk}(\ell, o, q)) \implies o \le |L|$.
Every `DoViewChange`, sent or started from, covers every op its sender
acknowledged in the view the body is from. This is the clause the truncation defect
would break.
