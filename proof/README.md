...
All three parts are checked in Lean: `Vsr.Inv.init`, `Vsr.Inv.step`, and
`Vsr.Inv.prefixAgreement` with `Vsr.Inv.durability`, assembled into
`Vsr.safety` in `lean/Vsr/Preserve/Step.lean`. Preservation is done one
handler at a time, and [`preservation.md`](preservation.md) records what
the induction needed beyond the clauses first written down.
...
| everything else in `Inv` | the paper (`docs/paper`) | proved: `Inv.init`, `Inv.step` for every handler, `Inv.prefixAgreement`, `Inv.durability`, `safety` |

The paper in `docs/paper` is now the hand proof, and its appendix on the
mechanised proof maps its clauses and lemmas to the Lean names. What is
not proved anywhere but on paper: linearizability (the model has no
clients). Liveness is not treated.
