...
proof reduces to one case per handler, showing that it preserves
...
Sections~\ref{sec:normal} to~\ref{sec:recovery}, subject to one rule
that precedes every handler: a replica whose status is \recovering{}
handles only \msg{RecoveryResponse} and the idle step, and drops every
other message unchanged. Everything the replica sends, replies, and
...
replica handles nothing but \msg{RecoveryResponse} is a guard of every
other handler, and is what makes ``not recovering'' a precondition
below.
...
\begin{iface}
  \item[Precondition] $\Inv$; a \msg{Recovery} is handled only by a
    replica that is not recovering.
...
every handler of Algorithms~\ref{alg:normal}, \ref{alg:viewchange},
...
primary's log. Until then it handles nothing else, by the first rule of
