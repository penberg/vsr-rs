...
\newcommand{\Local}{\textsc{Local-consistency}}
\newcommand{\WellFormed}{\textsc{Message-consistency}}
\newcommand{\OneLog}{\textsc{View-agreement}}
\newcommand{\CommitsBacked}{\textsc{Commits-backed}}
\newcommand{\Survival}{\textsc{Survival}}
...
\begin{enumerate}[nosep,label=(\roman*)]
  \item \emph{Prefix agreement.} If $i < k_a$ and $i < k_b$ then
    $L_a[i] = L_b[i]$.
  \item \emph{Durability.} For every $r \in \mathcal{R}$ and
    $i < k_r$, at least $|\mathcal{R}| + 1 - Q$ replicas in
    $\mathcal{R}$ hold $L_r[i]$ at index $i$.
...
one another. \OneLog: within a view, an index determines its entry, so every
fragment of a view's log that the history holds agrees with every other. \CommitsBacked: every commit number is
justified by a quorum of acknowledgements. \Survival: whatever was
committed in a view is held, at the same index, by every later view. The lemma
...
committed at index $i$ in some view $u < v$, then the vote of $V$ from
which $v$'s log was chosen holds $e$ at $i$.
\end{lemma}
...
client identity, request number, operation, indexed from zero. $L[i]$ is
the entry at index $i$, $L[a{:}b]$ the entries at indices $a, \dots,
b-1$, and $L \cdot e$ the log with $e$ appended. The \emph{op number} of
$L[i]$ is $i+1$.
\end{definition}
...
\label{def:frag}
$\Frag(v, \mathit{off}, L)$: the history holds $L$ as a piece of view
$v$'s log beginning at index $\mathit{off}$. One rule per kind of message
that carries log:
\[
\begin{array}{ll}
...
$\Holds(v, i, e)$: there are $\mathit{off} \le i$ and $L$ with
$\Frag(v, \mathit{off}, L)$ and $L[i - \mathit{off}] = e$.
\end{definition}

\begin{definition}[Acknowledged]
\leavevmode
\begin{enumerate}[nosep,label=(\roman*)]
  \item $\Acked(v, i, q)$: $q = \primary(v)$, or the history holds
    $\msg{PrepareOk}(v, o, q)$ for some $o > i$.
  \item $\QuorumAcked(v, i)$: there are $Q$ distinct identities $q < N$
    with $\Acked(v, i, q)$.
\end{enumerate}
\end{definition}

\begin{definition}[Committed]
$\Committed(v, i, e)$: $\Holds(v, i, e)$ and $\QuorumAcked(v, i)$.
\end{definition}

\begin{definition}[Backed]
$\Backed(v, k)$: for every $i < k$,
\[
\exists\, e,\ v' \le v.\quad \Committed(v', i, e) \wedge \Holds(v, i, e).
\]
\end{definition}
...
\leavevmode
\begin{enumerate}[nosep,label=(\roman*)]
  \item Within a view, an index determines its entry:
    $\Holds(v, i, e) \wedge \Holds(v, i, e') \Rightarrow e = e'$.
  \item Every replica's log agrees with the fragments of the view it
    came from: $L_r[i] = e \wedge \Holds(\ell_r, i, e') \Rightarrow e = e'$.
\end{enumerate}
\end{definition}

\begin{remark}
\OneLog{} says nothing across views: index $i$ may hold different
entries in views $4$ and $5$, and \Survival{} governs which of them
matter. It is the property Raft calls log matching, with views for
terms and message fragments alongside replica logs, and it is what lets
the rest of the invariant say ``the entry at index $i$ in view $v$'' and
mean one thing.
\end{remark}
...
If $\Committed(v', i, e)$ and $v' < v$, then every \msg{StartView} log
of $v$, every vote from $v$, every recovery state of $v$, every
\msg{NewState} segment of $v$ that reaches $i$, every \msg{Prepare} of
$v$ for op $i+1$, every started vote from $v$, and every non-recovering
replica with $\ell_r = v$ holds $e$ at $i$.
\end{definition}
...
\begin{enumerate}[nosep,label=(\alph*)]
  ...
  \item \emph{Votes cover acknowledgements.} Every started vote covers
    every op its sender acknowledged in the vote's view.
  ...
  \item \emph{Recovery covers acknowledgements.} A recovery response
    answering a recovering replica covers every op that replica
...
$L_r$ & log of $r$, a sequence of entries indexed from $0$ \\
$n_r$ & $|L_r|$, the op number: the op number of $L_r[i]$ is $i+1$ \\
$k_r$ & commit number: $L_r[0], \dots, L_r[k_r - 1]$ are committed \\
...
$L[i]$, $L[a{:}b]$, $L \cdot e$ & entry at $i$; entries $a, \dots, b-1$; append \\
...
\addlinespace
$\Frag(v, \mathit{off}, L)$ & the history holds $L$ as view $v$'s log from index $\mathit{off}$ \\
$\Holds(v, i, e)$ & some fragment of view $v$ has $e$ at index $i$ \\
$\Acked(v, i, q)$ & replica $q$ acknowledged index $i$ in view $v$ \\
$\QuorumAcked(v, i)$ & $Q$ distinct replicas acknowledged $i$ in $v$ \\
$\Committed(v, i, e)$ & $\Holds(v, i, e)$ and $\QuorumAcked(v, i)$ \\
$\Backed(v, k)$ & every index below $k$ is committed with the entry $v$ holds \\
...
\begin{iface}
  ...
  \item[Effect] Appends $e$ at index $n_r$, records its own
    acknowledgement of op $n_r + 1$, and sends
    $\msg{Prepare}(v_r, n_r+1, e, k_r)$ to the others.
...
\begin{iface}
  ...
  \item[Guarantee] $\Inv$. If the quorum completed, then
    $\QuorumAcked(v_r, j)$ holds for every $j < n$, and hence
    $\Backed(v_r, n)$.
\end{iface}
\end{lemma}
...
\begin{iface}
  \item[Precondition] $\Inv$; the replica is normal in $v = v_r$ and
    $n \le n_r$.
  \item[Effect] Sends $\msg{NewState}(v, L_r[n{:}], n, n_r, k_r)$ to $i$.
  \item[Guarantee] $\Inv$. The new fragment $\Frag(v, n, L_r[n{:}])$
...
\begin{iface}
  ...
  \item[Effect] In \transfer{} status, appends the part of $L$ past
    ...
    replaces $L_r[k_r{:}]$ by $L$, commits up to $k$, becomes normal in
...
\begin{iface}
  ...
  \item[Guarantee] $\Inv$. A vote sent from $r$ is $\Frag(\ell_r, 0,
    L_r)$ and covers every op $r$ acknowledged in $\ell_r$.
\end{iface}
\end{lemma}
...
\label{lem:committed}
If $\Inv(\Sigma)$ and $i < k_r$, there are $e$ and $v' \le \ell_r$ with
$L_r[i] = e$, $\Committed(v', i, e)$, and $\Holds(\ell_r, i, e)$.
\end{lemma}
\begin{proof}
\CommitsBacked gives $e$, $v'$, and $\Holds(\ell_r, i, e)$; \OneLog identifies
$e$ with $L_r[i]$.
\end{proof}

\begin{lemma}[Later views hold earlier commits]
\label{lem:later}
If $\Inv(\Sigma)$, $\Committed(v', i, e)$, $v' < v$, and
$\Holds(v, i, e')$, then $e' = e$.
\end{lemma}
\begin{proof}
...
Theorem~\ref{thm:inductive}. \emph{Prefix agreement:} for $i < k_a, k_b$,
...
$\sigma_r$ is the result of applying the operations of $L_r[0{:}k_r]$, in
...
$L[i]$ to the state produced by $L[0{:}i]$, which by \CommitsBacked was
committed before the reply was sent. A retransmitted request is answered
...
$L$. Order the operations by their index in $L$: every reply is
consistent with that order, and an operation's index is assigned after
its request was issued and before its reply was sent, so the order
respects real time.
\end{proof}
...
\Procedure{InstallLog}{$L$}
  ...
  \State $L_r \gets L$; rebuild $C_r$ from $L$, keeping replies for indices below $k_r$
\EndProcedure
...
The new entry is at index $n_r$, and by clause (d) every fragment of
$v_r$ ends at or before $n_r$, so \OneLog holds for the new fragment.
...
\msg{PrepareOk} for op $n$ in $v_r$, so $|A_r[n]| = Q$ gives
$\QuorumAcked(v_r, j)$ for all $j < n$. The entries at those indices are
held by $v_r$ (they are the primary's own fragments), so
$\Committed(v_r, j, L_r[j])$ and $\Backed(v_r, n)$; \CommitsBacked follows for
the new $k_r = n$.
\end{proof}
...
its log by \OneLog. While catching up, the kept prefix $L_r[{:}k_r]$ is
backed in $\ell_r < v$ by \CommitsBacked, so by \Survival the fragment of $v$
holds the same entries at those indices; the installed suffix is the
...
\On{idle} \why{lem:h-idle}
  \If{$\status_r = \normal \wedge \mathrm{isPrimary}(r)$}
    ...
    \State send $\msg{Prepare}(v_r, n, L_r[n-1], k_r)$ to others for each $n = k_r+1, \dots, n_r$ \dep{app:normal}
  \ElsIf{$\status_r = \normal$}
...
\begin{lemma}[The chosen log survives]
\label{lem:chosen}
If $\Inv(\Sigma)$, $(v, V) \in \started$, and $\Committed(u, i, e)$ with
$u < v$, then the best vote of $V$ holds $e$ at $i$.
\end{lemma}
\begin{proof}
Strong induction on $v$. The quorum that acknowledged $i$ in $u$ and the
senders of $V$ share a replica $r$ by Lemma~\ref{lem:quorum}. Let $\ell$ be
the last normal view in $r$'s vote; $\ell \ge u$ by
Lemma~\ref{lem:acks}. If $\ell = u$, $r$'s vote holds $e$ at $i$ by
clause (c) and \OneLog. If $u < \ell < v$, $r$ adopted the
\msg{StartView} of $\ell$, which by the induction hypothesis at $\ell$
holds $e$ at $i$, and $r$'s vote extends it. In either case $r$'s vote
holds $e$ at $i$; the best vote has a last normal view at least $\ell$
and a log at least as long, and by \Survival at every view between, holds
$e$ at $i$ too. This is Lemma~\ref{lem:chosen-intro}.
\end{proof}
...
new $(v_r, V)$. For $u < v_r$ and $\Committed(u, i, e)$,
Lemma~\ref{lem:chosen} gives $\mathit{best}.L[i] = e$, which is \Survival
...
at the same indices by \Survival, so the new $k_r = \max$ is backed. The
...
The primary's re-sent \msg{Prepare} for op $n$ carries $L_r[n-1]$, a
fragment of $v_r$ that the history already holds by clause (d) and
...
paper). A \emph{client table} records, per client, the number of its
...
\msg{NewState}$(v, L, a, b, k)$ with the log from index $a$ to $b$, which
it appends (\S5.2). A replica that learns of a later view than its own
