...
\newcommand{\WellFormed}{\textsc{Well-formed}}
...
each other. \OneLog: within a view, an index determines its entry, so every
...
Every message in $\sent$ is well formed: a \msg{StartView} or
...
both. Every message it sends is well formed (Definition~\ref{def:wellformed}).
\end{lemma}
\begin{proof}
