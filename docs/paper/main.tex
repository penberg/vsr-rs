...
\newcommand{\dvc}{\msg{DoViewChange}}
...
to~\ref{sec:recovery} give the actions as pseudocode with every erratum
...
the bound is a majority. A recovering replica holds nothing and sends
no \dvc{}, so it counts on neither side.
...
We prove the theorem with an inductive invariant: a statement $\Inv$
about a cluster state that holds in the initial cluster, that every step
preserves, and that implies the theorem. Induction on the number of
steps then gives the theorem in every reachable state, and the whole
proof reduces to one case per action, showing that it preserves
$\Inv$. This is the standard method for safety of a distributed
protocol, and the whole difficulty is in choosing $\Inv$. It must be
stronger than the theorem, because the theorem alone is not preserved by
every step: at the step where an acknowledgement completes a quorum and
an entry becomes committed, showing the new entry durable needs to know
which replicas acknowledged it, and prefix agreement and durability say
nothing about acknowledgements. $\Inv$ (Section~\ref{sec:invariant})
records what the induction needs.
...
things happened: a \dvc{} is sent after the acknowledgements its
sender made, a view is started at most once, a primary's log in its view never
...
quorum, and view $u > v$ was started from a quorum $V$ of \dvc{}
messages, and every \dvc{} in $V$ from a view strictly between $v$ and
$u$ holds $e$ at $n$. Then the \dvc{} of $V$ from which $u$'s log was
chosen holds $e$ at $n$.
\end{lemma}

The two quorums intersect in a replica $x$. The \dvc{} $x$ sent for
$u$ was sent after $x$ acknowledged $n$ in $v$, so it is from a view at
least $v$; if from $v$ itself, it covers the acknowledgement; if from a
later view, the hypothesis applies. The \dvc{} the new primary chooses
is at least as good as $x$'s, by the ordering it uses, and holds $e$
too. A strong
...
can go backwards; without the ban on truncation a \dvc{} need not
cover what its sender acknowledged; without monotone commits \CommitsBacked{}
...
action as a lemma and proves the theorem from those interfaces, before
any action is shown. Sections~\ref{sec:normal} to~\ref{sec:recovery}
give the actions as pseudocode, one section per sub-protocol, each with
...
identities; a flag $\mathit{dvcSent}_r$; the \dvc{} messages $V_r$
received as new primary, mapping identities to their bodies; a nonce
$x_r$; recovery responses $R_r$ mapping identities to
...
\end{tabular}
\end{center}
The \emph{body} of a \dvc{} is the triple $(\ell, L, k)$ it carries:
...
every view $v$ some primary started and the \dvc{} messages $V$ it
chose the log from. $\Sent(\mathit{to}, m)$ means $(\mathit{to}, m) \in \sent$.
...
in $\sent$ carries. The replica takes the action of
Sections~\ref{sec:normal} to~\ref{sec:recovery} for that message,
subject to one rule that precedes every action: a replica whose status
is \recovering{} processes only \msg{RecoveryResponse} and the idle
step, and drops every other message unchanged. Everything the replica sends, replies, and
starts is added to $\sent$, $\mathit{replies}$, and $\started$, which
therefore only grow.
\end{definition}
...
\begin{definition}[DoViewChange of]
\label{def:dvcof}
$\mathrm{Dvc}(u, x, d)$: replica $x$ sent a \dvc{} with body
$d = (\ell, L, k)$ for view $u$. Either the history holds
$\dvc(u, x, \ell, L, |L|, k)$, or $(u, V) \in \started$ with
$(x, d) \in V$. The second case covers the new primary's own \dvc{},
...
\mathrm{Dvc}(\_, \_, (\ell, L, \_)) & \Rightarrow \Frag(\ell, 0, L)
\end{array}
\]
\end{definition}

\begin{remark}
A \dvc{} is a fragment of the view its sender was \emph{last normal
in}, $\ell$, not of the view it is for. A replica catching up with a view
...
\label{lem:mono}
$\mathrm{Dvc}$, $\Frag$, $\Holds$, $\Acked$, $\Committed$, and $\Backed$
...
$\sent$ carrying a commit number $k$ in view $v$, $\Backed(v, k)$, a
\dvc{} being judged by its last normal view.
\end{definition}

\begin{definition}[\Survival]
If $\Committed(v', n, e)$ and $v' < v$, then every \msg{StartView} log
of $v$, every \dvc{} from $v$, every recovery state of $v$, every
...
\begin{enumerate}[nosep,label=(\alph*)]
  \item \emph{Views only rise.} No message in $\sent$, and no recorded
    \dvc{}, carries a view above its sender's current view. A replica in
    \viewchange{} status has $\ell_r < v_r$; a \dvc{} for $u$ is from a
  ...
  \item \emph{Chosen from a quorum, once.} A view is started at most
    ...
    $Q$ \dvc{} messages, from distinct replicas, it was started from.
    Every whole log of a started view, and every \msg{NewState} of it,
    reaches at least as far as that best \dvc{}'s log.
  \item \emph{DoViewChanges cover acknowledgements.} A \dvc{} from $x$
    covers every op-number $x$ acknowledged in its view $\ell$; if $x$ is
    the primary of $\ell$, it covers every fragment of $\ell$.
  \item \emph{DoViewChanges come after acknowledgements.} If $x$
    acknowledged in view $v$, or is the primary of a started view $v$,
    and sent a \dvc{} for $u > v$, that \dvc{} is from a view
    $\ell \ge v$.
...
$A_r, S_r, V_r, R_r, C_r$ & the maps and sets of Definition~\ref{def:replica} \\
\addlinespace
...
$(\ell, L, k)$ & a \dvc{} body: last normal view, log, commit number \\
\addlinespace
$\Sigma$ & a cluster state \\
$\sent$ & every $(\mathit{to}, m)$ ever sent; never shrinks \\
$\started$ & every $(v, V)$: view $v$ was started from the \dvc{}s $V$ \\
$\mathcal{R}$ & the replicas whose status is not recovering \\
$\Sent(\mathit{to}, m)$ & $(\mathit{to}, m) \in \sent$ \\
$\mathrm{Dvc}(u, x, d)$ & $x$'s \dvc{} for view $u$ has body $d$, sent or recorded \\
\addlinespace
...
This section states what each action guarantees and proves the theorem
from those guarantees. The actions themselves are in
...
assumed. A replica's fields are written by its actions, and a message's
...
includes the two facts every action relies on to run at all: that its
...
requirements of the actions, in the sense that a subroutine has
preconditions its callers must meet. The callers here are the action
...
\Local{} is proved once for every action, and \WellFormed{} once for
every send. Everything above them in the invariant is proved the same
way, one action at a time, from the interface lemmas below.
...
(\Survival) falls on the sender. Three actions send acknowledgements,
\msg{Prepare}, \msg{NewState}, and \msg{StartView}, and each of their
interface lemmas carries that obligation. The primary's action on
\msg{PrepareOk} only reads a quorum that already exists.

\subsection{Interfaces of the actions}

Every interface lemma has the same shape. \textsc{Precondition} is what
the action may assume, always including $\Inv$. \textsc{Effect} is what
...
proof uses. An action whose guard fails has no effect and trivially
preserves $\Inv$. The rule of Definition~\ref{def:step} that a recovering
replica processes nothing but \msg{RecoveryResponse} is a guard of every
other action, and is what makes ``not recovering'' a precondition
below.

One guarantee is shared by every action and needs no assumption about
the message received, so it is stated once.

\begin{lemma}[Local]
\label{lem:local}
Every action of Sections~\ref{sec:normal} to~\ref{sec:recovery}, given
...
The remaining lemmas are one per action.
...
\label{lem:h-svc}
\begin{iface}
  \item[Precondition] $\Inv$; the replica is not recovering; $v \ge v_r$.
  \item[Effect] If $v > v_r$, adopts $v$ in \viewchange{} status and
    announces it. Records the sender. On $f$ senders, sends its \dvc{}
    with body $(\ell_r, L_r, k_r)$ to $\primary(v_r)$, or records it if
    it is the primary. On a \dvc{} as primary, records it; on the $Q$th,
    starts the view (Lemma~\ref{lem:h-start}).
  \item[Guarantee] $\Inv$. A \dvc{} sent by $r$ satisfies clauses (a), (c),
...
\begin{iface}
  \item[Precondition] $\Inv$; the replica is $\primary(v_r)$ in
    \viewchange{} status and holds $Q$ \dvc{} messages $V$ from distinct
    replicas,
    each sent to it for $v_r$ or its own.
  \item[Effect] Adds $(v_r, V)$ to $\started$; installs the log $L$ of
    the best \dvc{}; commits up to the greatest $k$ in $V$; becomes normal
...
\begin{iface}
  \item[Precondition] $\Inv$; the replica is not recovering; $v > v_r$,
    or $v = v_r$ in \viewchange{} status.
...
\begin{iface}
  \item[Precondition] $\Inv$; only a replica that is not recovering
    processes a \msg{Recovery}.
...
\begin{iface}
  ...
  \item[Effect] Re-sends: the primary its \msg{Commit} and every
    ...
    its \msg{StartViewChange} and, if it has sent one, its \dvc{}. A
    backup or
...
\end{theorem}
\begin{proof}
Lemma~\ref{lem:init} for the initial cluster. A step is one action at
one replica; every action is covered by exactly one of
...
Algorithm~\ref{alg:normal} the actions.
...
\Procedure{EnterNormal}{}
  \State $\status_r \gets \normal$;\ $\ell_r \gets v_r$;\ $S_r, V_r \gets \emptyset$;\ $\mathit{dvcSent}_r, \mathit{catching}_r \gets \mathrm{false}$
\EndProcedure
...
\Procedure{CatchUp}{$v$}
  ...
  \State $v_r \gets v$;\ $\status_r \gets \viewchange$;\ $\mathit{catching}_r \gets \mathrm{true}$;\ $S_r, V_r \gets \emptyset$;\ $\mathit{dvcSent}_r \gets \mathrm{false}$
...
\begin{algorithm}[tp]
\caption{Normal operation and state transfer. The first rule applies to
every action of Algorithms~\ref{alg:normal}, \ref{alg:viewchange},
and~\ref{alg:recovery}.}
\label{alg:normal}
\begin{algorithmic}[1]
\On{any message other than $\msg{RecoveryResponse}$} \Comment{Definition~\ref{def:step}}
  \Guard{$\status_r \ne \recovering$}
...
started $v$; a \dvc{} from $v$ is the log of a replica last normal in $v$,
...
started from $V$; by the induction hypothesis every \dvc{} in $V$ from
a view strictly between $v$ and $u$ holds $L_r[m]$ at $m$, since such a
\dvc{} is a fragment of that view. Lemma~\ref{lem:chosen} then gives
that the best \dvc{} of $V$ holds $L_r[m]$ at $m$. By clause (b) the
\msg{StartView} of $u$ extends the best \dvc{}, so it holds $L_r[m]$ at
...
says so. A replica that hears $f$ others say so sends the new primary a
\dvc{}: its log, tagged with the view the log came from. The new primary
takes the log from the \dvc{} with the latest such view, longest on a
tie,
...
Algorithm~\ref{alg:viewchange} the actions, and Algorithm~\ref{alg:idle}
...
\Procedure{StartViewChange}{$v$}
  \State $v_r \gets v$;\ $\status_r \gets \viewchange$;\ $\mathit{catching}_r \gets \mathrm{false}$;\ $S_r, V_r \gets \emptyset$;\ $\mathit{dvcSent}_r \gets \mathrm{false}$
  \Send{$\msg{StartViewChange}(v, \mathrm{id}_r)$}{others}
  \State \Call{MaybeSendDoViewChange}{}
\EndProcedure
\Procedure{MaybeSendDoViewChange}{}
  \If{$\status_r = \viewchange \wedge \lnot\mathit{catching}_r \wedge \lnot\mathit{dvcSent}_r \wedge |S_r| \ge f$}
    \State $\mathit{dvcSent}_r \gets \mathrm{true}$;\ \Call{SendDoViewChange}{}
  \EndIf
\EndProcedure
\Procedure{SendDoViewChange}{} \why{lem:h-svc}
  \If{$\primary(v_r) = \mathrm{id}_r$} \Call{RecordDoViewChange}{$\mathrm{id}_r$, $(\ell_r, L_r, k_r)$}
  \Else\ send $\msg{DoViewChange}(v_r, \mathrm{id}_r, \ell_r, L_r, n_r, k_r)$ to the primary
  \EndIf
\EndProcedure
\Procedure{RecordDoViewChange}{$i$, $d$} \why{lem:h-start}
  \State $V_r[i] \gets d$
  \If{$|V_r| < Q$} \Return \EndIf
  \State $\mathit{best} \gets$ the body in $V_r$ with the greatest $(\ell, |L|)$ lexicographically, ties to the highest identity \why{lem:chosen}
...
\On{$\msg{StartViewChange}(v, i)$} \why{lem:h-svc}
  ...
  \If{$\status_r = \viewchange$} $S_r \gets S_r \cup \{i\}$;\ \Call{MaybeSendDoViewChange}{}
...
\On{$\msg{DoViewChange}(v, i, \ell, L, n, k)$} \why{lem:h-svc}
  ...
  \Else\ \Call{RecordDoViewChange}{$i$, $(\ell, L, k)$}
  \EndIf
...
\On{idle} \why{lem:h-idle}
  ...
  \Else
    \Send{$\msg{StartViewChange}(v_r, \mathrm{id}_r)$}{others};\ \textbf{if} $\mathit{dvcSent}_r$ \textbf{then} \Call{SendDoViewChange}{}
  \EndIf
\end{algorithmic}
\end{algorithm}
...
Let $u > v$, and let $V$ be $Q$ \dvc{} messages for $u$ from distinct
replicas, each satisfying clauses (c) and (d), such that every \dvc{}
in $V$ from a view $\ell$ with $v < \ell < u$ holds $e$ at $n$. Then the
best \dvc{} of $V$, by $(\ell, |L|)$, holds $e$ at $n$.
\end{lemma}
\begin{proof}
By Lemma~\ref{lem:quorum} some $x \in X$ sent a \dvc{} with body
$d = (\ell, L, k)$ in $V$. By clause (d), $\ell \ge v$: $x$ either sent a \msg{PrepareOk} in
$v$ before voting, or is the primary of $v$, which was started since it
holds a fragment. If $\ell = v$: by clause (c) the \dvc{} covers $n$, as
...
The $Q$ \dvc{} messages are from distinct replicas and $v_r$ was not started
before, as its primary was not normal in it, so clause (b) holds for the
new $(v_r, V)$. For $u < v_r$ and $\Committed(u, n, e)$, every \dvc{} in
...
$V$ is backed in its \dvc{}'s $\ell < v_r$ by \CommitsBacked, and by
...
re-sent \dvc{} is the case of Lemma~\ref{lem:h-svc}. Starting view
...
primary's log. Until then it processes nothing else, by the first rule of
Algorithm~\ref{alg:normal}. Its \msg{Recovery}
carries the persisted view, so replicas behind it move forward first.
Algorithm~\ref{alg:recovery} gives the actions.
...
Appendix~\ref{app:lean}. What has not: no human has yet checked, line by line, that the
pseudocode and definitions printed in this paper are the model the Lean
proves, or that the hand proofs follow the Lean. The correspondence in
Appendix~\ref{app:lean} is the agent's. The author takes responsibility
for the whole.
...
\begin{departure}{read as an increment, ``advances its view-number'' lets a primary count \dvc{} messages sent for different views together}
...
actions keep the protocol's idle counters, and the environment chooses
...
model sets a flag instead of stopping, so every action is total and
...
Definition~\ref{def:dvcof} & \texttt{DvcOf} \\
...
each action's lemma above. The clauses of Definition~\ref{def:inv} are
...
(c) DoViewChanges cover acknowledgements & \texttt{DvcCoversAcks}, \texttt{DvcPrimaryCovers} \\
(d) DoViewChanges come after acknowledgements & \texttt{DvcAfterAcks}, \texttt{DvcAfterOwnView} \\
...
\texttt{StepOK} into \texttt{Inv} of the new cluster. An action is a
