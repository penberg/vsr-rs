...
mod tests {
    ...
    fn test_normal_operation() {
        ...
        let client = Client::new(config, replica_tx.clone());
    ...
    fn test_idle() {
        ...
        assert_eq!(5, accumulator);
        tick();
        let accumulator_b = (sm.accumulator.lock().unwrap()).to_owned();
        let accumulator_c = (sm.accumulator.lock().unwrap()).to_owned();
