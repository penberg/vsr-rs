...
/- Something of view `v` exists: a fragment, a message only a replica
normal in `v` sends, or a replica that was last normal in `v` and
remembers it. -/
ghost relation in_view (v : view) :=
  (∃ n e, frag v n e) ∨ (∃ k, m_commit v k) ∨ (∃ a b k, m_new_state v a b k) ∨
  (∃ n q, m_prepare_ok v n q) ∨ (∃ x q n k, m_rr_state x q v n k) ∨
  (∃ r, last_normal r = v ∧ status r ≠ recovering)
...
the argument covers. -/
procedure start_view_from (r : replica) (bq : replica) (kmax : idx) {
  let v := cur_view r
...
action recv_recovery (r : replica) (q : replica) (x : nonce) (v : view) {
  ...
  require q ≠ r    -- `Recovery` goes to the others
  require status r ≠ recovering
  if vw.lt (cur_view r) v then
    start_view_change r v
  else if status r = normal then
...
invariant [recovering_forgot]
  status R = recovering → ¬ acks R N Q ∧ ¬ votes R Q ∧ ¬ svc_from R Q ∧ ¬ catching_up R
invariant [view_change_after_last_normal] status R = view_change → vw.lt (last_normal R) (cur_view R)
invariant [state_transfer_not_primary] status R = state_transfer → ¬ is_primary R
invariant [normal_no_votes] status R = normal → ¬ votes R Q
...
invariant [wf_dvc] m_dvc V Q → ix.le (vote_commit V Q) (vote_len V Q) ∧ vw.lt (vote_lnv V Q) V
invariant [wf_rr_state] m_rr_state X Q V N K → ix.le K N ∧ m_rr X Q V ∧ primary V = Q

/- The logs messages carry are as long as they say. -/
invariant [start_view_dense] m_start_view V L K ∧ ix.lt ix.zero M ∧ ix.le M L → ∃ E, frag V M E
invariant [vote_dense]
  m_dvc V Q ∧ ix.lt ix.zero M ∧ ix.le M (vote_len V Q) → ∃ E, frag (vote_lnv V Q) M E
invariant [rr_state_dense] m_rr_state X Q V L K ∧ ix.lt ix.zero M ∧ ix.le M L → ∃ E, frag V M E
invariant [new_state_dense] m_new_state V A B K ∧ ix.lt A M ∧ ix.le M B → ∃ E, frag V M E

/- Nonces are fresh, and nobody answers its own recovery. -/
invariant [recovery_nonce_used] m_recovery Q X V → nonce_used X
invariant [rr_nonce_used] m_rr X Q V → nonce_used X
invariant [recovering_nonce_used] status R = recovering → nonce_used (rec_nonce R)
invariant [rr_not_self] m_rr X Q V ∧ m_recovery P X W0 → P ≠ Q
...
invariant [survives_start_view]
  committed W0 N E ∧ vw.lt W0 V ∧ m_start_view V L K → frag V N E ∧ ix.le N L
invariant [survives_vote]
  committed W0 N E ∧ vw.lt W0 V ∧ m_dvc V2 Q ∧ vote_lnv V2 Q = V → frag V N E ∧ ix.le N (vote_len V2 Q)
invariant [survives_rr]
  committed W0 N E ∧ vw.lt W0 V ∧ m_rr_state X Q V L K → frag V N E ∧ ix.le N L
...
invariant [primary_longest_frag]
  last_normal P = V ∧ primary V = P ∧ status P ≠ recovering ∧ frag V N E → ix.le N (log_len P)
invariant [primary_vote_covers]
  m_dvc V Q ∧ primary (vote_lnv V Q) = Q ∧ frag (vote_lnv V Q) N E → ix.le N (vote_len V Q)
invariant [voted_once] m_dvc V Q ∧ cur_view Q = V ∧ status Q = view_change → dvc_sent Q
invariant [primary_after_start_view]
  m_start_view V L K ∧ cur_view (primary V) = V →
    status (primary V) = normal ∨ status (primary V) = recovering
...
invariant [start_view_chosen]
  m_start_view V L K ∧ V ≠ vw.zero → ∃ Q : quorum, ∀ p, member p Q → chosen V p
...
invariant [prepare_below_primary_view] m_prepare V N E K → vw.le V (cur_view (primary V))
invariant [commit_below_primary_view] m_commit V K → vw.le V (cur_view (primary V))
invariant [new_state_below_primary_view] m_new_state V A B K → vw.le V (cur_view (primary V))
invariant [start_view_below_primary_view] m_start_view V L K → vw.le V (cur_view (primary V))
invariant [recovery_covers_acks]
  status Q = recovering ∧ m_rr_state (rec_nonce Q) P V N K ∧ m_prepare_ok V O Q → ix.le O N
invariant [started_views] in_view V ∧ V ≠ vw.zero → ∃ L K, m_start_view V L K

#gen_spec

/-! ## Testing before proving -/

/- The module's assumptions are satisfiable. -/
sat trace [initial_state] { }

/- A commit is reachable: request, one acknowledgement, quorum. -/

/- A view change is reachable. -/

/- Three replicas, three views, op numbers 0..2, two distinct entries, two
recoveries. `primary v = v` is `v mod 3`. -/


#check_action client_request
#check_action recv_prepare
#check_action recv_prepare_ok
#check_action commit_quorum
#check_action recv_commit
#check_action recv_get_state
#check_action recv_new_state
#check_action recv_start_view_change
#check_action send_do_view_change
#check_action recv_do_view_change
#check_action start_view
#check_action recv_start_view
#check_action timeout
#check_action primary_idle
#check_action crash_recover
#check_action recv_recovery
#check_action recv_recovery_response
#check_action finish_recovery
end RunCheck
