...
sat trace [can_commit] {
  ...
  recv_prepare_ok
  commit_quorum
  assert (∃ r, ix.lt ix.zero (commit r))
}

/- A view change is reachable. -/
sat trace [can_change_view] {
  timeout
  recv_start_view_change
  send_do_view_change
  recv_do_view_change
  start_view
  assert (∃ r, cur_view r ≠ vw.zero ∧ status r = normal)
}

/- Three replicas, three views, op numbers 0..2, two distinct entries, two
recoveries. `primary v = v` is `v mod 3`. -/
#model_check
  { replica := Fin 3, view := Fin 3, idx := Fin 3, entry := Fin 2,
    quorum := Quorum 3, nonce := Fin 2 }
  { primary := fun v => v, member := fun r q => r ∈ q }
