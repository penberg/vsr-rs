...
/-- A `DoViewChange` from `x` for view `u` with body `d`: in `sent`, or in
the set a started view was chosen from. The primary's own is only in the
latter. -/
def DvcOf (s : System Op Output St) (u : ViewNumber) (x : ReplicaId) (d : DoViewChange Op) : Prop :=
  (∃ to, Sent s to (.doViewChange u x d.lastNormalView d.log d.log.length d.commitNumber)) ∨
  (∃ dvcs, (u, dvcs) ∈ s.started ∧ (x, d) ∈ dvcs)
...
def Survives (s : System Op Output St) : Prop :=
  ∀ v' i e, Committed s v' i e → ∀ v, v' < v →
    ...
    (∀ u x d, DvcOf s u x d → d.lastNormalView = v → d.log[i]? = some e) ∧
...
ops in its own view, each acknowledger once, and each a replica. -/
...
/-- The primary of a view holds the longest log of the view, for as long
as that view is its last normal one and it has not lost its memory: every
fragment of the view, every log of a replica last normal in it, and every
op acknowledged in it is within the primary's log. Its log in the view only
grows while it is normal, and is frozen once it moves on. -/
def PrimaryLongest (s : System Op Output St) : Prop :=
  ∀ p ∈ s.replicas, p.status ≠ .recovering → p.selfId = s.config.primaryId p.lastNormalView →
    (∀ off log, Frag s p.lastNormalView off log → off + log.length ≤ p.log.length) ∧
    (∀ q ∈ s.replicas, q.lastNormalView = p.lastNormalView → q.status ≠ .recovering →
      q.log.length ≤ p.log.length) ∧
    (∀ to o q, Sent s to (.prepareOk p.lastNormalView o q) → o ≤ p.log.length)
...
/-- Every `DoViewChange`, sent or started from, covers every op its sender
acknowledged in the view it is from. This is the clause truncation in state
transfer would break. -/
def DvcCoversAcks (s : System Op Output St) : Prop :=
  ∀ u x d, DvcOf s u x d → ∀ to o, Sent s to (.prepareOk d.lastNormalView o x) → o ≤ d.log.length

/-- A `DoViewChange` is from a view before the one it is for: it is sent in
view-change status, where the last normal view is behind. -/
def DvcBehind (s : System Op Output St) : Prop :=
  ∀ u x d, DvcOf s u x d → d.lastNormalView < u

/-- A `DoViewChange` for `u` from `x` means `x` has reached view `u`, and
views only rise. Like `MessagesBelowView`, but also for the primary's own,
which is never sent. -/
def DvcBelowView (s : System Op Output St) : Prop :=
  ∀ u x d, DvcOf s u x d → ∀ r ∈ s.replicas, r.selfId = x → u ≤ r.viewNumber

/-- A `DoViewChange` for `u` is from a view no earlier than any view below
`u` its sender acknowledged in: the acknowledgement came first, and the
last normal view only rises. -/
def DvcAfterAcks (s : System Op Output St) : Prop :=
  ∀ u x d, DvcOf s u x d → ∀ to v o, Sent s to (.prepareOk v o x) → v < u → v ≤ d.lastNormalView

/-- The same for the primary's implicit acknowledgement: a `DoViewChange`
for `u` from the primary of a started view `v < u` is from a view no
earlier than `v`, since the primary was normal in `v` before it could move
to `u`. -/
def DvcAfterOwnView (s : System Op Output St) : Prop :=
  ∀ u x d, DvcOf s u x d → ∀ v dvcs, (v, dvcs) ∈ s.started → x = s.config.primaryId v →
    v < u → v ≤ d.lastNormalView

/-- A `DoViewChange` from the primary of the view it is from covers every
fragment of that view: the primary's log in its view is the longest and
never shrinks. -/
def DvcPrimaryCovers (s : System Op Output St) : Prop :=
  ∀ u x d, DvcOf s u x d → x = s.config.primaryId d.lastNormalView →
    ∀ off log, Frag s d.lastNormalView off log → off + log.length ≤ d.log.length

/-- Every replica in view-change status was last normal in an earlier
view. -/
def ViewChangeBehind (s : System Op Output St) : Prop :=
  ∀ r ∈ s.replicas, r.status = .viewChange → r.lastNormalView < r.viewNumber

/-- The primary of a started view has been normal in it: its last normal
view is at least that view. -/
def PrimaryStartedNormal (s : System Op Output St) : Prop :=
  ∀ v dvcs, (v, dvcs) ∈ s.started → ∀ p ∈ s.replicas, p.selfId = s.config.primaryId v →
    v ≤ p.lastNormalView

/-- A view is started at most once. -/
def StartedOnce (s : System Op Output St) : Prop :=
  ∀ v d1 d2, (v, d1) ∈ s.started → (v, d2) ∈ s.started → d1 = d2

/-- Every whole log of a started view, and every `NewState` of it, reaches
at least as far as the log the view was started from: the best of its
DoViewChanges. Everything in a view descends from that log by appending.
Together with `OneLogPerView` this says they all extend it. -/
def ViewExtendsBase (s : System Op Output St) : Prop :=
  ∀ v dvcs b, (v, dvcs) ∈ s.started → Replica.bestDoViewChange dvcs = some b →
    (∀ u x d, DvcOf s u x d → d.lastNormalView = v → b.log.length ≤ d.log.length) ∧
    (∀ to n r st, Sent s to (.recoveryResponse v n r (some st)) → b.log.length ≤ st.log.length) ∧
    (∀ to log a e k, Sent s to (.newState v log a e k) → b.log.length ≤ e) ∧
    (∀ r ∈ s.replicas, r.lastNormalView = v → r.status ≠ .recovering → b.log.length ≤ r.log.length)

/-- An acknowledgement in a view above zero means the view was started:
the acknowledger was normal in it. -/
def AcksStarted (s : System Op Output St) : Prop :=
  ∀ to u o q, Sent s to (.prepareOk u o q) → 0 < u → ∃ dvcs, (u, dvcs) ∈ s.started

/-- No `StartViewChange` is for view zero: a view change goes to a later
view. -/
def StartViewChangePos (s : System Op Output St) : Prop :=
  ∀ to v q, Sent s to (.startViewChange v q) → 0 < v

/-- A replica in state transfer is not its view's primary: only a backup
asks the primary for state. -/
def TransferNotPrimary (s : System Op Output St) : Prop :=
  ∀ r ∈ s.replicas, r.status = .stateTransfer → r.selfId ≠ s.config.primaryId r.viewNumber

/-- A recovery state comes from the primary of its view. -/
def RecoveryStateFromPrimary (s : System Op Output St) : Prop :=
  ∀ to v n q st, Sent s to (.recoveryResponse v n q (some st)) → q = s.config.primaryId v

/-- A recovering replica did not answer its own recovery. -/
def RecoveryResponseNotSelf (s : System Op Output St) : Prop :=
  ∀ to v n p st, Sent s to (.recoveryResponse v n p st) →
    ∀ q ∈ s.replicas, q.status = .recovering → q.recoveryNonce = n → p ≠ q.selfId

/-- There are as many replicas as the configuration says. -/
def ReplicaCount (s : System Op Output St) : Prop := s.replicas.length = s.config.replicaCount

/-- Every message that names its sender names a replica. -/
def SenderIds (s : System Op Output St) : Prop :=
  ∀ to (msg : Message Op), Sent s to msg →
    match msg with
...
/-- The DoViewChanges a view was started from are from replicas. -/
def StartedIds (s : System Op Output St) : Prop :=
  ∀ v dvcs, (v, dvcs) ∈ s.started → ∀ x d, (x, d) ∈ dvcs → x < s.config.replicaCount

/-- What a replica in view-change status has recorded as the new primary:
each DoViewChange is one that was sent to it for its view, or its own, and
they are keyed by sender in order. -/
def RecordedDvcs (s : System Op Output St) : Prop :=
  ∀ r ∈ s.replicas, r.status = .viewChange →
...
/-- What a recovering replica has recorded: responses to its recovery. -/
def RecordedRRs (s : System Op Output St) : Prop :=
  ∀ r ∈ s.replicas, r.status = .recovering →
...
/-- Every `RecoveryResponse` answers a `Recovery` with its nonce. -/
def RecoveryResponseNonce (s : System Op Output St) : Prop :=
  ∀ to v n r st, Sent s to (.recoveryResponse v n r st) → ∃ to' i v', Sent s to' (.recovery i n v')
...
structure Inv (s : System Op Output St) : Prop where
  ...
  dvcCovers : DvcCoversAcks s
  dvcBehind : DvcBehind s
  dvcBelow : DvcBelowView s
  dvcAfterAcks : DvcAfterAcks s
  dvcAfterOwn : DvcAfterOwnView s
  dvcPrimary : DvcPrimaryCovers s
  vcBehind : ViewChangeBehind s
  primaryStarted : PrimaryStartedNormal s
  startedOnce : StartedOnce s
  extendsBase : ViewExtendsBase s
  rrNonce : RecoveryResponseNonce s
  acksStarted : AcksStarted s
  svcPos : StartViewChangePos s
  transferNotPrimary : TransferNotPrimary s
  rrPrimary : RecoveryStateFromPrimary s
  rrNotSelf : RecoveryResponseNotSelf s
  count : ReplicaCount s
  senderIds : SenderIds s
  startedIds : StartedIds s
  recordedDvcs : RecordedDvcs s
  recordedRRs : RecordedRRs s
...
theorem DvcOf.mono {u x d} (h : DvcOf s u x d) : DvcOf s' u x d := by
  rcases h with ⟨to, h⟩ | ⟨dvcs, h1, h2⟩
  · exact Or.inl ⟨to, Sent.mono hsent h⟩
  · exact Or.inr ⟨dvcs, hstarted _ h1, h2⟩
...
theorem MsgBacked.mono {msg : Message Op} (h : MsgBacked s msg) : MsgBacked s' msg := by
...
theorem DvcOf.init {config : Config} {sm : St} {u x} {d : DoViewChange Op}
    (h : DvcOf (System.init config sm : System Op Output St) u x d) : False := by
...
theorem Inv.init (config : Config) (sm : St) (htwo : 2 ≤ config.replicaCount) :
  ...
  dvcCovers := fun _ _ _ h => (DvcOf.init h).elim
  dvcBehind := fun _ _ _ h => (DvcOf.init h).elim
  dvcBelow := fun _ _ _ h => (DvcOf.init h).elim
  dvcAfterAcks := fun _ _ _ h => (DvcOf.init h).elim
  dvcAfterOwn := fun _ _ _ h => (DvcOf.init h).elim
  dvcPrimary := fun _ _ _ h => (DvcOf.init h).elim
  vcBehind := by
    intro r hr hs
    simp only [System.init, List.mem_map] at hr
    obtain ⟨_, _, rfl⟩ := hr
    simp [Replica.new] at hs
  primaryStarted := by intro v dvcs h; simp [System.init] at h
  startedOnce := by intro v d1 d2 h; simp [System.init] at h
  extendsBase := by intro v dvcs b h; simp [System.init] at h
  rrNonce := fun _ _ _ _ _ h => (Sent.init h).elim
  acksStarted := fun _ _ _ _ h => (Sent.init h).elim
  svcPos := fun _ _ _ h => (Sent.init h).elim
  transferNotPrimary := by
    intro r hr hs
    simp only [System.init, List.mem_map] at hr
    obtain ⟨_, _, rfl⟩ := hr
    simp [Replica.new] at hs
  rrPrimary := fun _ _ _ _ _ h => (Sent.init h).elim
  rrNotSelf := fun _ _ _ _ _ h => (Sent.init h).elim
  count := by simp [ReplicaCount, System.init, Config.replicas]
  senderIds := fun _ _ h => (Sent.init h).elim
  startedIds := by intro v dvcs h; simp [System.init] at h
  recordedDvcs := by
    intro r hr hs
    simp only [System.init, List.mem_map] at hr
    obtain ⟨_, _, rfl⟩ := hr
...
theorem Inv.survives_holds {s : System Op Output St} (hinv : Inv s) {v' v i} {e e' : LogEntry Op}
  ...
  obtain ⟨hsv, hdvc, hrec, hnew, hprep, _⟩ := hinv.survives v' i e hc v hlt
  cases hf with
  | prepare h =>
    rename_i o c n op k
  ...
  | @dvc to u x _ log o k h =>
    simp only [Nat.sub_zero] at hget
    have hwf : log.length = o := (hinv.wf _ h).1
    subst hwf
    have : DvcOf s u x ⟨v, log, k⟩ := Or.inl ⟨to, h⟩
    exact Option.some.inj (hget.symm.trans (hdvc _ _ _ this rfl))
  | recovery h =>
    simp only [Nat.sub_zero] at hget
    exact Option.some.inj (hget.symm.trans (hrec _ _ _ _ h))
  | started h hv =>
    simp only [Nat.sub_zero] at hget
    exact Option.some.inj (hget.symm.trans (hdvc _ _ _ (Or.inr ⟨_, h, hv⟩) rfl))
